...
from cards.utils import NotifyUtil, EmailUtil
...
class TestCardsTask(TestCase):
    ...
    @mock.patch.object(EmailUtil, 'send_email', mock.MagicMock(return_value=None), create=True)
    ...
    @mock.patch.object(EmailUtil, 'send_email', mock.MagicMock(return_value=None), create=True)
    @mock.patch('cards.tasks.datetime')
    @mock.patch('cards.tasks.NotifyUtil.send_notification')
    ...
    @mock.patch('cards.tasks.EmailUtil.send_email', create=True)
